...
def generate_dracor_stats(inferior=1650, superior=1725, corpus = corpusFolder):
    ...
    # Plotting
    graph_type = 'verses'
    max_x = 2500 # 5 for acts, 70 for scenes
    ticks_every = 100
    plt.bar(range(max_x), [verse_numbers[i] for i in range(max_x)], align = 'center', width=90)
    plt.title(
        f'Repartition of the number of {graph_type} {"(rounded to closest hundreds)" if graph_type == "verses" else "" } per Dracor play with a print date between {inferior} and {superior}')
    plt.xlabel(f'Number of {graph_type}')
    plt.xticks(range(0, max_x, ticks_every))
